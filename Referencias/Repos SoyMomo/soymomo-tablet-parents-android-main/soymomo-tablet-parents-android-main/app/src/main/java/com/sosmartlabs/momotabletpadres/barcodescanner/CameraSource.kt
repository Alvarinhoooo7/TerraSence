...
class CameraSource(private val graphicOverlay: GraphicOverlay) {
    ...
    internal fun stop() {
        ...
        processingThread?.let {
            try {
                ...
                with(Firebase.crashlytics) {
                    log("Frame processing thread interrupted on stop")
                    recordException(e)
                }
            }
        ...
        camera?.let {
            ...
            try {
                ...
                with(Firebase.crashlytics) {
                    log("Failed to clear camera preview")
                    recordException(e)
                }
            }
            it.release()
    ...
    private fun setPreviewAndPictureSize(camera: Camera, parameters: Camera.Parameters) {
       ...
       /* val sizePair: CameraSizePair = PreferenceUtils.getPreferredPreviewSize(camera) ?: run {
        ...
        override fun run() {
            ...
            while (true) {
                synchronized(lock) {
                    while (active && pendingFrameData == null) {
                        try {
                            ...
                            with(Firebase.crashlytics) {
                                log("Frame processing loop terminated.")
                                recordException(e)
                            }
                            return
                        }
                ...
                try {
                    ...
                    with(Firebase.crashlytics) {
                        log("Exception thrown from receiver")
                        recordException(t)
                    }
                } finally {
                    data?.let {
    ...
    companion object {
        ...
        private const val MAX_CAMERA_PREVIEW_WIDTH = 1920
        private const val DEFAULT_REQUESTED_CAMERA_PREVIEW_WIDTH = 1280
        private const val DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT = 720
