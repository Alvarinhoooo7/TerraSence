...
class LocationHistoryGeocoderHelper @Inject constructor(
    ...
    // ConcurrentHashMap forbids null values, so coordinates that resolve to no
    // address are cached as the NO_ADDRESS sentinel to avoid re-geocoding them.
    ...
    suspend fun getAddressForLocation(latitude: Double, longitude: Double): String? {
        ...
        // Check cache first (NO_ADDRESS marks a coordinate already known to have no address)
        addressCache[cacheKey]?.let { cached ->
            return if (cached === NO_ADDRESS) null else cached
        }
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                // Cache the result (NO_ADDRESS when null, to avoid retrying failed lookups)
                addressCache[cacheKey] = result ?: NO_ADDRESS
                ...
                // Cache NO_ADDRESS to avoid retrying
                addressCache[cacheKey] = NO_ADDRESS
                null
            }
        }
    }
    ...
    private companion object {
        // Negative-cache sentinel for coordinates known to have no address, so
        // they are not re-geocoded. ConcurrentHashMap forbids null values;
        // compared by reference identity (===), so it never collides with a
        // real geocoded address (which is always built at runtime).
        val NO_ADDRESS = "__NO_ADDRESS__"
    }
}
